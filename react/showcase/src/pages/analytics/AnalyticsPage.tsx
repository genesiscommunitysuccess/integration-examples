...
import { CriteriaSegmentedControlOption, Serialisers, CriteriaSegmentedControl } from '@genesislcap/foundation-criteria';
...
const helperCriteriaEvent = (criteriaSegmentedControl: any, chartDataResource: any): () => void => {
...
const AnaliticsPage = () => {
  const [ ribbonButtonsConfig, setRibbonButtonsConfig ] = useState<AnalyticChartRegistration[]>([]);
  ...
  const addChartToLayout = ({ registration: registration, title: title }: AnalyticChartRegistration) => {
  ...
  const setLayoutChildrenProps = () => {
    const roseLayoutComponent = layoutComponentsMap.get(LayoutComponentsNames.ROSE);
    ...
    const pieLayoutComponent = layoutComponentsMap.get(LayoutComponentsNames.PIE);
  ...
  const handleEmmitsInsideLayout = () => {
    ...
    onDestroyActions.push(helperCriteriaEvent(criteriaSegmentedControl, pieChartDataResource));
  }
  ...
  const upadateRibbonButtons = () => {
    if (analyticsLayout.current) {
      ...
      setRibbonButtonsConfig(
        Array.from(analyticsLayout.current.querySelectorAll('zero-layout-item'))
  ...
  useEffect(() => {
    ...
    if (analyticsLayout.current) {
      ...
      analyticsLayout.current.addEventListener(LayoutEmitEvents.firstLoaded, () => {
        setChartsConfigAndData();
        setLayoutChildrenProps();
      ...
      if (loadLayoutButton.current) {
        loadLayoutButton.current.addEventListener('click', loadLayoutButtonHandler);
      }

      const handleClickLoadDefaultLayout = () => loadPredefinedLayout('DEFAULT_EXTENDED_LAYOUT');
      ...
      const handleClickLoadCompactLayout = () => loadPredefinedLayout('COMPACT_LAYOUT');
      ...
      return () => {
        ...
        if (saveButton.current) {
          saveButton.current.removeEventListener('click', saveLayoutButtonHandler);
        }

        if (loadLayoutButton.current) {
          loadLayoutButton.current.removeEventListener('click', loadLayoutButtonHandler);
        }
    ...
    <zero-flex-layout class={style['analytics-page']}>
      <zero-tabs>
        ...
        <zero-tab-panel slot="tabpanel">
            <div
              className="container"
              style={{ display: 'grid', gridTemplateColumns: '1fr', gridAutoRows: 'minmax(5vh, auto)' }}
            >
              ...
              <div  style={{ display: 'block', position: 'relative', gridRowStart: 2, gridRowEnd: 19 }}>
                <zero-layout ref={analyticsLayout} auto-save-key="client-app-analytics-layout">
                  <zero-layout-region>
        ...
        <zero-tab-panel slot="tabpanel">
          <zero-flex-layout class="flex-row justify-center items-center">
            ...
            <zero-g2plot-chart ref={pieChartInTab} class="container" type="pie"></zero-g2plot-chart>
            <zero-flex-layout class="flex-column justify-center items-center">
              ...
              <zero-g2plot-chart ref={pieChart2InTab} class="container" type="pie">
                <chart-datasource
        ...
        <zero-tab-panel slot="tabpanel">
            <zero-g2plot-chart ref={dualaxesChartInTab} type="dualaxes"></zero-g2plot-chart>
        </zero-tab-panel>
        <zero-tab-panel slot="tabpanel">
        ...
        <zero-tab-panel slot="tabpanel">
            <zero-g2plot-chart ref={roseChartInTab} type="rose"></zero-g2plot-chart>
        </zero-tab-panel>
        <zero-tab-panel slot="tabpanel">
